...
from .db_operations import get_groups_to_add, get_unread_count, get_user_by_pk,\
    get_message_by_id, save_text_message, mark_message_as_read
from .message_types import MessageTypes, MessageTypeMessageRead, MessageTypeTextMessage, OutgoingEventMessageRead, \
    OutgoingEventNewTextMessage, OutgoingEventNewUnreadCount, OutgoingEventMessageIdCreated, OutgoingEventIsTyping,\
    OutgoingEventStoppedTyping, OutgoingEventWentOnline, OutgoingEventWentOffline

from .errors import ErrorTypes, ErrorDescription
from private_chat.models import ChatMessage
# from private_chat.serializers
...
class PrivateChatConsumer(AsyncWebsocketConsumer):
    ...
    def __inti__(self, *args, **kwargs):
        ...
        self.sender_username: str = None
        self.error: Optional[ErrorDescription] = None
        # self

    async def _after_message_save(self, msg: ChatMessage, rid: int, user_pk: str):
        ev = OutgoingEventMessageIdCreated(
            random_id=rid, db_id=msg.id)._asdict()
        logger.info(
    ...
    async def connect(self):
        if self.scope["user"].is_authenticated:
            ...
            await self.accept()
            chats = await get_groups_to_add(self.user)
            logger.info(
                f"User {self.user.pk} connected, sending 'user_went_online' to {chats} dialog groups"
            )
            for c in chats:
                if str(c) != self.group_name:
                    await self.channel_layer.group_send(str(c), OutgoingEventWentOnline(user_pk=str(self.user.pk))._asdict())
        else:
            logger.info(
    ...
    async def disconnect(self, close_code):
        if close_code != UNAUTH_REJECT_CODE and getattr(self, 'user', None) is not None:
            ...
            chats = await get_groups_to_add(self.user)
            logger.info(
                f"User {self.user.pk} disconnected, sending 'user_went_offline' to {chats} chat groups")
            for c in chats:
                await self.channel_layer.group_send(str(c), OutgoingEventWentOffline(user_pk=str(self.user.pk))._asdict())
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        try:
            ...
            if not ('msg_type' in text_data_json):
                self.error = (ErrorTypes.MessageParsingError,
                              "msg_type not present in json")
            else:
                msg_type = text_data_json['msg_type']
                if not isinstance(msg_type, int):
                    self.error = (ErrorTypes.MessageParsingError,
                                  "msg_type is not an int")
                else:
                    try:
                        msg_type_case: MessageTypes = MessageTypes(msg_type)
                        self.error = await self.handle_received_message(msg_type=msg_type_case, data=text_data_json)
    ...
    async def handle_received_message(self, msg_type: MessageTypes, data: Dict[str, str]) -> Optional[ErrorDescription]:
        ...
        if msg_type == MessageTypes.WentOffline \
            ...
            if msg_type == MessageTypes.IsTyping:
                chats = await get_groups_to_add(self.user)
                logger.info(
                    f"User {self.user.pk} is typing, sending 'is_typing' to {chats} chat groups")
                for c in chats:
                    if str(c) != self.group_name:
                        await self.channel_layer.group_send(str(c), OutgoingEventIsTyping(user_pk=str(self.user.pk))._asdict())
                return None
            elif msg_type == MessageTypes.TypingStopped:
                chats = await get_groups_to_add(self.user)
                logger.info(
                    f"User {self.user.pk} has stopped typing, sending 'stopped_typing' to {chats} chat groups")
                for c in chats:
                    if str(c) != self.group_name:
                        await self.channel_layer.group_send(str(c), OutgoingEventStoppedTyping(user_pk=str(self.user.pk))._asdict())
                    return None
                ...
                if 'user_pk' not in data:
                    ...
                    recipient: Optional[AbstractUser] = await get_user_by_pk(user_pk)
                    logger.info(
            ...
            #         return ErrorTypes.InvalidRandomId, "'random_id' should be negative"
                ...
                if 'text' not in data:
                    ...
                    await self.channel_layer.group_send(self.group_name, OutgoingEventNewTextMessage(random_id=rid,
                                                                                             text=text,
                                                                                             sender=self.group_name,
                                                                                             receiver=user_pk,
                                                                                             sender_username=self.sender_username)._asdict())
                    ...
                    if not recipient:
                        ...
                        msg = await save_text_message(text, _from=self.user, to=recipient)
                        await self._after_message_save(msg=msg, rid=rid, user_pk=user_pk)
